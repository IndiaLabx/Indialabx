...
class _WorkspaceScreenState extends ConsumerState<WorkspaceScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: _buildAppBar(context, workspaceState, notifier),
      body: Stack(
        children: [
          if (workspaceState.pages.isEmpty)
            _EmptyWorkspace(notifier: notifier)
          ...
          if (workspaceState.mode == WorkspaceMode.focus)
            _FocusPageIndicator(workspaceState: workspaceState),

          const FluidDeck(),

          if (workspaceState.isProcessing)
            _ProcessingOverlay(workspaceState: workspaceState),
        ],
      ),
...
class _EmptyWorkspace extends StatelessWidget {
  ...
  const _EmptyWorkspace({required this.notifier});
...
class _WorkspaceGridView extends StatelessWidget {
  final WorkspaceState workspaceState;
  final WorkspaceController notifier;
  final Function(int) onPageSelected;

  const _WorkspaceGridView({
...
class _WorkspaceFocusView extends StatelessWidget {
  final WorkspaceState workspaceState;
  final WorkspaceController notifier;
  final PageController pageController;

  const _WorkspaceFocusView({
    required this.workspaceState,
    required this.notifier,
...
class _FocusPageIndicator extends StatelessWidget {
  final WorkspaceState workspaceState;

  const _FocusPageIndicator({required this.workspaceState});
...
class _ProcessingOverlay extends StatelessWidget {
  ...
  const _ProcessingOverlay({required this.workspaceState});
...
class _WorkspaceFab extends StatelessWidget {
  ...
  const _WorkspaceFab({required this.workspaceState, required this.onPressed});
  ...
  Widget build(BuildContext context) {
    return AnimatedPadding(
      ...
      padding: EdgeInsets.only(
        bottom: workspaceState.activeTool == ActiveToolTier.none ? 70.0 : 230.0,
      ),
